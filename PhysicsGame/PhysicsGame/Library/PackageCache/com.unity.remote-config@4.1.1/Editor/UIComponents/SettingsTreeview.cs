                ...
                case 2:
                    ...
                    switch (item.data["rs"]["type"].Value<string>())
                    {
                        ...
                        case "json":
                            ...
                            string schemaId = item.data["rs"]["schemaId"].ToString();
                            ...
                            {
                                m_SettingsJsonModal = SettingsJsonModal.CreateInstance(inputJsonValue, item.data["rs"]["key"].ToString(), item.data["metadata"]["entityId"].ToString(), schemaId);
                            }
                                ...
                                if (m_SettingsJsonModal != null )
                                {
                                    for (int i = 0; i < activeSettingsList.Count; i++)
                                    {
                                        if (activeSettingsList[i]["metadata"]["entityId"].ToString() == m_SettingsJsonModal.keyEntityId)
                                        {
                                            ...
                                            newSetting["rs"]["schemaId"] = m_SettingsJsonModal.currentSchemaId;
