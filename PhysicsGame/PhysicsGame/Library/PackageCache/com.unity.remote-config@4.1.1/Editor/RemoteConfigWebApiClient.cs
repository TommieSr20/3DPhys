...
namespace Unity.RemoteConfig.Editor
{
    ...
    public static class RemoteConfigWebApiClient
    {
        ...
        /// </summary>
        public static event Action<JArray> fetchSchemasFinished;
        ...
        /// </summary>
        /// <param name="cloudProjectId">Cloud Project ID for this Unity application.</param>
        /// <param name="configId">ID of the config that we want to fetch schemas for</param>
        /// <param name="responseParseErrorCallback">Optional callback for parsing errors.</param>
        public static void FetchSchemas(string cloudProjectId, string configId, Action<Exception> responseParseErrorCallback = null)
        {
            ...
            {
                m_WebRequestEnumerators.Add(_FetchSchemas(cloudProjectId, configId, responseParseErrorCallback));
                AddUpdateListenerIfNeeded();
            }
        }
        ...
        private static IEnumerator<AsyncOperation> _FetchEnvironments(string cloudProjectId, Action<Exception> responseParseErrorCallback = null)
        {
            ...
            using (var request = Authorize(UnityWebRequest.Get(url)))
            {
        ...
        private static IEnumerator<AsyncOperation> _FetchConfigs(string cloudProjectId, string environmentId, Action<Exception> responseParseErrorCallback = null)
        {
            ...
            using (var request = Authorize(UnityWebRequest.Get(remoteSettingsUrl)))
            {
                yield return request.SendWebRequest();
        ...
        private static IEnumerator<AsyncOperation> _PutConfig(string cloudProjectId, string configId, string payload)
        {
            ...
            using (var request = Authorize(UnityWebRequest.Put(url, payload)))
            {
                yield return request.SendWebRequest();
        ...
        private static IEnumerator<AsyncOperation> _PostConfig(string cloudProjectId, string payload, Action<Exception> responseParseErrorCallback = null)
        {
            ...
            using (var request = Authorize(new UnityWebRequest(url, UnityWebRequest.kHttpVerbPOST)))
            {
        ...
        {
            string url = string.Format(RemoteConfigEnvConf.putConfigPath, cloudProjectId, configId);
            using (var request = Authorize(UnityWebRequest.Delete(url)))
            {
                yield return request.SendWebRequest();
        ...
        private static IEnumerator<AsyncOperation> _FetchSchemas(string cloudProjectId, string configId, Action<Exception> responseParseErrorCallback = null)
        {
            string schemasUrl = string.Format(RemoteConfigEnvConf.getSchemasPath, cloudProjectId, configId);
            using (var request = Authorize(UnityWebRequest.Get(schemasUrl)))
            {
                yield return request.SendWebRequest();
                ...
                {
                    var error = ParseResponseToJError(request);
                    Debug.LogWarning("Failed to fetch schemas: " + request.error + "\n" + error["message"].Value<string>());
                    rcRequestFailed?.Invoke(request.responseCode, request.error);
                    yield break;
                }

                string schemasJson = request.downloadHandler.text;
                ParseSchemas(schemasJson, responseParseErrorCallback);
            }
        }
        ...
        private static void ParseSchemas(string json, Action<Exception> responseParseErrorCallback)
        {
            JArray schemas = new JArray();
            try
            {
                schemas = (JArray)(JObject.Parse(json)["schemas"]);
            }
            catch (Exception e)
            {
                Debug.LogWarning("Remote Config response was not valid JSON:\n" + json + "\n" + e);
            ...
            fetchSchemasFinished?.Invoke(schemas);
        }
        ...
        private static JObject ParsePostConfigResponse(string json, Action<Exception> responseParseErrorCallback)
        {
            ...
            {
                Debug.LogWarning("POST config response was not valid JSON:\n" + json + "\n" + e);
                responseParseErrorCallback?.Invoke(e);
            }
            return response;
        }
