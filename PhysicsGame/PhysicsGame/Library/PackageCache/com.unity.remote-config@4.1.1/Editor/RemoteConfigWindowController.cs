...
namespace Unity.RemoteConfig.Editor
{
    internal class RemoteConfigWindowController
    {
        ...
        private void OnFetchRemoteSettingsFinished(JObject config)
        {
            ...
            RemoteConfigWebApiClient.fetchSchemasFinished += OnFetchSchemasFinished;
            try
            {
                RemoteConfigWebApiClient.FetchSchemas(Application.cloudProjectId, m_DataStore.configId);
            }
            catch
            {
                RemoteConfigWebApiClient.fetchSchemasFinished -= OnFetchSchemasFinished;
        ...
        private void OnFetchSchemasFinished(JArray schemas)
        {
            DoCleanUp();
            RemoteConfigWebApiClient.fetchSchemasFinished -= OnFetchSchemasFinished;
            RemoteConfigDataStore.schemas = schemas;
        }
